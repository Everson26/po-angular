...
export abstract class PoChartBaseComponent {
   ...
   *
   * É possível mesclar gráficos dos tipos linha e coluna, veja detalhamento na interface `PoChartSerie`.
   *
  ...
  @Input('p-series') set series(value: Array<PoChartSerie> | PoChartGaugeSerie) {
    ...
    if (Array.isArray(this._series) && this._series.length) {
      ...
      this.validateSerieAndAddType(this._series);
    } else {
  ...
  private validateSerieAndAddType(series: Array<PoChartSerie>): void {
    ...
    this.chartSeries = series
      ...
      .map((serie, index) => {
        this.chartType =
          index === 0 && (<any>Object).values(PoChartType).includes(serie.type)
            ? serie.type
